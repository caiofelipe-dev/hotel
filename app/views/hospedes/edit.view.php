<div class="card">
    <div class="card-header"><h5>Editar Hóspede</h5></div>
    <div class="card-body">
        <?php
            ...
            $isObject = is_object($hospede);
            $get = function($key) use ($hospede, $isObject, $old) {
                if (!empty($old[$key])) return $old[$key];
                if ($isObject) return $hospede->$key ?? null;
                return $hospede[$key] ?? null;
            };
        ?>
        ...
        <?php endif; ?>
        <form method="post" action="<?= route('hospedes.update')->setParamns(['id' => $hospede->id]) ?>">
            <div class="row">
            ...
            <div class="row">
                <div class="mb-3 col-md-4">
                    ...
                    <input name="telefone" class="form-control<?= $isInvalid?>" required value="<?= htmlspecialchars($get('telefone') ?? '') ?>" placeholder="(11) 99999-9999">
                    <?php if(!empty($fieldErrs)): foreach($fieldErrs as $msg): ?>
                        <div class="invalid-feedback d-block"><?= htmlspecialchars($msg) ?></div>
                    <?php endforeach; endif; ?>
                </div>
                ...
                <div class="mb-3 col-md-4">
                    ...
                    <input name="data_nascimento" type="date" class="form-control<?= $isInvalid?>" required value="<?= htmlspecialchars($get('data_nascimento') ?? '') ?>">
                    <?php if(!empty($fieldErrs)): foreach($fieldErrs as $msg): ?>
                        <div class="invalid-feedback d-block"><?= htmlspecialchars($msg) ?></div>
                    <?php endforeach; endif; ?>
                </div>
            ...
            <div class="row">
                <div class="mb-3 col-md-6">
                    ...
                    <input name="nacionalidade" class="form-control<?= $isInvalid?>" required value="<?= htmlspecialchars($get('nacionalidade') ?? '') ?>" placeholder="Brasileira">
                    <?php if(!empty($fieldErrs)): foreach($fieldErrs as $msg): ?>
                        <div class="invalid-feedback d-block"><?= htmlspecialchars($msg) ?></div>
                    <?php endforeach; endif; ?>
                </div>
            ...
            <div class="mb-3">
                ...
                <textarea name="endereco" class="form-control<?= $isInvalid?>" required placeholder="Rua, número, complemento, cidade"><?= htmlspecialchars($get('endereco') ?? '') ?></textarea>
                <?php if(!empty($fieldErrs)): foreach($fieldErrs as $msg): ?>
                    <div class="invalid-feedback d-block"><?= htmlspecialchars($msg) ?></div>
                <?php endforeach; endif; ?>
            </div>
